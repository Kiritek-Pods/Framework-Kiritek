...
1. **Spec primero** — usa `/speckit.specify` + `/speckit.plan` ([Spec Kit](https://github.com/github/spec-kit) oficial) para generar spec+plan y obtén aprobación explícita antes de delegar nada.
...
No te saltes pasos por presión de "rapidez". Si el usuario pide un fix trivial de una línea, se puede saltar el spec formal — pero igual pasa por QA y git-workflow.
